...
## Соответствие логике ВКР

В реализации сохранена логика метода:
...
→ граф знаний
→ кандидаты комплексных проблем
→ индекс комплексности
→ LLM/heuristic-интерпретация
→ интерфейс городского ИИ-помощника
```

Комплексная проблема не задается заранее и не сводится к одному слову. Она формируется как связанный кандидат: группа обращений, сущностей, действий, акторов, территорий и отношений, подтвержденная расчетными признаками.

---

## Что можно доработать дальше

- Добавить векторный поиск по текстам обращений.
- Подключить полноценный RAG-контур для вопросно-ответного режима.
- Добавить интерактивную визуализацию локального подграфа проблемы.
- Разделить типы сущностей на более строгую онтологию городской среды.
- Добавить экспорт карточек проблем в `.docx` или `.pdf`.
- Добавить тесты для пайплайна и проверки JSONL-контракта.

---

## Кратко

Главный файл для UI и LLM:
